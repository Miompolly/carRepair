    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        <form onSubmit={handleCreate} className="mb-6 grid gap-3 rounded bg-white p-4 shadow md:grid-cols-4">
          <select name="serviceRecord" value={form.serviceRecord} onChange={handleChange} className="rounded border px-3 py-2" required>
            <option value="">Select Service Record</option>
            {serviceRecords.map((record) => (
              <option key={record._id} value={record._id}>
                {record.car?.plateNumber || "No Car"} - {record.service?.serviceName || "No Service"}
              </option>
            ))}
          </select>
              ...
              {payments.map((payment) => (
                <tr key={payment._id} className="border-t">
                  <td className="px-4 py-3">
                    {payment.serviceRecord?.car?.plateNumber || "No Car"} -{" "}
                    {payment.serviceRecord?.service?.serviceName || "No Service"}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button type="button" onClick={() => handleDelete(payment._id)} className="rounded bg-red-500 px-3 py-1 text-white hover:bg-red-600">
                      Delete
                    </button>
                  </td>
                </tr>
